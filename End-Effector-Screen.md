...
<BR>`https://tiny-circuits.com/Downloads/ArduinoBoards/package_tinycircuits_index.json` on a separate line. (Note: The original Tinyscreen without the + is just an Arduino Uno clone, but the Tinyscreen+ is a SAMD processor, which is not supported by the base IDE)
